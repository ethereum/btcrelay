...
class TestBtcEth(object):
    ...
    def testTransfer(self):
        ...
        res = self.c.processTransaction(self.TX_STR.decode('hex'), self.TX_HASH, profiling=True)
    ...
    def testUntrustedCaller(self):
        res = self.c.processTransaction(self.TX_STR.decode('hex'), self.TX_HASH, sender=tester.k1)
        assert res == 0
    ...
    def testRelayCanClaimWithDifferentTx(self):
        ...
        res = self.c.processTransaction(self.TX_STR.decode('hex'), anotherHash, sender=tester.k1)
        assert res == 1
    ...
    def testRelayCanNotReclaim(self):
        ...
        res = self.c.processTransaction(self.TX_STR.decode('hex'), self.TX_HASH, sender=tester.k0)
        assert res == 1
        ...
        res = self.c.processTransaction(self.TX_STR.decode('hex'), self.TX_HASH)
        assert res == 0
