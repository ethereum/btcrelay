...
class TestFee(object):
    ...
    def testChargeOneRelayTx(self):
        ...
        hh = self.bulkStoreFrom300K(numHeader, keySender, addrSender, feeWei)
    ...
    def checkRelay(self, txStr, txIndex, btcAddr, hh, numHeader, keySender, addrSender):
        ...
        BTC_ETH = self.s.abi_contract(self.BTC_ETH_CONTRACT, sender=tester.k1)
        ...
        senderBal = self.s.block.get_balance(addrSender)
        balCaller = self.s.block.get_balance(tester.a0)
        res = self.c.relayTx(txStr, txHash, txIndex, siblings, txBlockHash, BTC_ETH.address, sender=keySender, value=self.FEE_VERIFY_TX, profiling=True)
