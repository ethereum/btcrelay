...
class TestTxVerify(object):
    ...
    def testRelayTx(self):
        ...
        assert 0 == self.c.relayTx(txStr.decode('hex'), txIndex, siblings, txBlockHash, BTC_ETH.address, sender=tester.k0)

        # owner of exchange contract should be able to reclaim
        assert 1 == self.c.relayTx(txStr.decode('hex'), txIndex, siblings, txBlockHash, BTC_ETH.address, sender=tester.k1)
    ...
    def testInsufficientBTCSent(self):
        ...
        res = self.c.relayTx(txStr.decode('hex'), txIndex, siblings, txBlockHash, BTC_ETH.address)
