...
class TestBtcTx(object):
    ...
    def testRelayTx(self):
        ...
        res = self.doRelayTx(header, hashes, 1, BTC_ETH.address, profiling=True)
    ...
    def doRelayTx(self, header, hashes, index, contract, profiling=False):
        numTx = len(hashes)
        # print('txStr='+hashes[index])
        ...
        path = self.indexToPath(index, nSibling)
        txBlockHash = int(header['hash'], 16)
        res = self.c.relayTx(tx, len(siblings), siblings, path, txBlockHash, contract, profiling=profiling)
        print('GAS: '+str(res['gas']))
        return res['output']
